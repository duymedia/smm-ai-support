        ...
        <div className="flex items-center gap-1.5 sm:gap-2">
          ...
          <div className="hidden md:block">
            <Button
              ...
              disabled={isPurging}
              icon={<Zap className={`w-3.5 h-3.5 text-amber-500 ${isPurging ? 'animate-spin' : ''}`} />}
            >
              Purge CDN
            </Button>
          </div>
          ...
          <div id="admin-currency-select-container" className="relative">
            <button
              ...
              className="flex items-center gap-2 px-3 py-1.5 rounded-full border border-slate-200 bg-slate-50 hover:bg-slate-100 hover:border-slate-300 text-xs font-bold text-slate-900 transition-all cursor-pointer shadow-2xs"
              title="Chọn đơn vị tiền tệ & Xem số dư"
            >
              <div className="flex items-center gap-1.5">
                <span className="text-slate-900 font-extrabold tracking-tight font-mono tabular-nums">
                  {formatMoney(user?.balance || 0)}
                </span>
                <span className="px-1.5 py-0.5 rounded-full text-[10px] font-bold bg-blue-100 text-blue-800 border border-blue-200/60 font-mono">
                  {currency}
                </span>
              </div>
          ...
          <div className="lg:hidden fixed inset-0 z-50 flex">
            ...
            <div className="relative flex-1 flex flex-col max-w-xs w-full bg-white z-50 shadow-2xl animate-in slide-in-from-left duration-200 h-full">
                      ...
                      {group.items.map((item) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            {item.badge && (
                              <Badge variant={isActive ? "neutral" : "brand"} pulse={item.badge === 'LIVE'} size="sm">
                                {item.badge}
                              </Badge>
                            )}
              ...
              <div className="p-4 border-t border-slate-200 bg-slate-50/70 shrink-0 space-y-3">
                ...
                >
                  {language === 'vi' ? 'Về Trang Người Dùng' : 'Back to User Portal'}
                </Button>
                ...
                <div className="p-3 rounded-2xl bg-white border border-slate-200 shadow-2xs space-y-2 text-xs">
                  ...
                  <div className="flex items-center justify-between text-[11px] text-slate-500 font-mono tabular-nums">
                    ...
                    <span className="text-slate-700 font-bold">1.2 GB / 4 GB</span>
                  </div>
